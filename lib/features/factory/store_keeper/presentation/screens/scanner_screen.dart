...
class _ScannerScreenState extends State<ScannerScreen>
  ...
  void _processScan(String code) {
    ...
    context.read<StoreKeeperBloc>().add(
      ScanCode(code: code, codeType: _lastScannedType),
    );
  ...
  double get _successRate =>
      _totalScanAttempts == 0 ? 1.0 : _successfulScans / _totalScanAttempts;
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Icon(
              needsAttention ? Icons.warning_amber : Icons.check_circle,
              size: 48.w,
              color: needsAttention ? AppColors.warning : AppColors.success,
            ),
            Gap(12.h),
            Text(
            ...
            Gap(8.h),
            Text(
              '$_successfulScans / $_totalScanAttempts scans',
            ...
            if (needsAttention)
              Container(
                ...
                child: Text(
                  'Below 95%. Check phone camera lens.',
            ...
            IconButton(
              icon: Icon(
                _successRate >= 0.95
                    ...
                    : Icons.warning_amber,
                color: _successRate >= 0.95
                    ? AppColors.success
                    : AppColors.warning,
              ),
              tooltip: 'Quality: ${(_successRate * 100).toStringAsFixed(0)}%',
              onPressed: _showQualityCheckDialog,
            ),
          IconButton(
            icon: Icon(
              _isBatchMode ? Icons.filter_none : Icons.filter_none_outlined,
              color: _isBatchMode ? AppColors.secondary : Colors.white,
            ),
            tooltip: _isBatchMode ? 'Batch ON' : 'Batch OFF',
            onPressed: () {
              setState(() {
                _isBatchMode = !_isBatchMode;
                if (!_isBatchMode) _batchCodes.clear();
              });
            },
          ),
          IconButton(
            icon: Icon(
              Icons.keyboard,
              color: _showManualEntry ? AppColors.accent : Colors.white,
            ),
      ...
      body: Stack(
        ...
        children: [
          MobileScanner(
            ...
            errorBuilder: (context, error) => _buildError(error.toString()),
          ),
          const ScannerOverlay(),
          if (_lastScannedCode != null) _buildLastScanned(),
          if (_isBatchMode && _batchCodes.isNotEmpty) _buildBatchCounter(),
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Gap(4.w),
              Text(
                '${_batchCodes.length} scanned',
                style: TextStyles.captionBold.copyWith(color: Colors.white),
              ),
            ],
          ),
        ),
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  const Icon(Icons.keyboard, color: AppColors.accent),
                  Gap(8.w),
                  Text('Enter Code Manually', style: TextStyles.heading6),
                  const Spacer(),
                  IconButton(
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Type or paste barcode / numeric code...',
    ...
    return Positioned(
      ...
      child: SafeArea(
        child: Row(
          ...
          children: [
            TorchButton(isTorchOn: _isTorchOn, onToggle: () => _toggleTorch()),
            Gap(24.w),
            if (_isBatchMode && _batchCodes.isNotEmpty)
            ...
            if (_isBatchMode && _batchCodes.isNotEmpty) Gap(16.w),
            FloatingActionButton(
              heroTag: 'scanner_close',
              onPressed: () {
                if (_isBatchMode && _batchCodes.isNotEmpty) {
                  context.pop(_batchCodes);
                } else {
                  context.pop();
                }
              },
      ...
      child: Center(
        child: Padding(
          ...
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              const Icon(Icons.camera_alt, size: 64, color: AppColors.error),
              Gap(16.h),
              Text(
                'Camera Unavailable',
