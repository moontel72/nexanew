...
class SubAdminListScreen extends StatelessWidget {
        ...
        builder: (ctx, state) {
          ...
          return Scaffold(
            ...
            body: state.subAdminListLoading
                ...
                ? _errorView(
                    state.subAdminListError!,
                    () => bloc.add(const FetchSubAdmins()),
                  )
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            Text(
                              '${state.subAdmins.length} assigned',
  ...
  Widget _statusChip(String status) {
    final c = status == 'deleted'
        ? AppColors.error
        : status == 'suspended'
        ? AppColors.warning
        : AppColors.success;
    final l = status == 'deleted'
        ? 'DELETED'
        : status == 'suspended'
        ? 'SUSPENDED'
        : 'ACTIVE';
  ...
  void _handleAction(
    ...
    switch (action) {
      case 'edit':
        _showEditDialog(ctx, sa, bloc);
        break;
      case 'toggle_status':
        bloc.add(ToggleSubAdminStatus(id));
        break;
      case 'change_vertical':
        _showChangeVerticalDialog(ctx, sa, bloc);
        break;
      case 'reset_password':
        _showResetPasswordDialog(ctx, sa, bloc);
        break;
      case 'delete':
        showDialog(
          context: ctx,
  ...
  void _showChangeVerticalDialog(
    BuildContext ctx,
    Map<String, dynamic> sa,
    SubAdminBloc bloc,
  ) {
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(dctx);
                bloc.add(
                  ChangeSubAdminVertical(
                    adminId: sa['id'] as String,
                    newVertical: selected,
                  ),
                );
              },
