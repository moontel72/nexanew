...
class _ConductorDashboardScreenState extends State<ConductorDashboardScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    if (token.isEmpty) { if (mounted) context.go('/bus-conductor/login'); return; }
    setState(() => _conductorName = prefs.getString('conductor_name') ?? 'Conductor');
    await _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final seats = (data['seats'] as List<dynamic>?)
          ?.map((s) => s is Map<String, dynamic> ? s : <String, dynamic>{'number': s.toString(), 'status': 'vacant'})
          .toList() ?? [];
      setState(() {
        _routeName = data['route_name']?.toString() ?? '--';
        _totalSeats = (data['total_seats'] as num?)?.toInt() ?? 52;
        _bookedSeats = seats.where((s) => s['status'] == 'booked').length;
        _seats = seats;
        _isLoading = false;
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Conductor Terminal', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 17)),
              ...
              : RefreshIndicator(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Welcome, $_conductorName', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
                      const Gap(4),
                      Text('Route: $_routeName', style: const TextStyle(fontSize: 14, color: AppColors.gray500)),
                      const Gap(24),

                      // Stats row
                      Row(children: [
                        Expanded(child: _statCard('Booked', '$_bookedSeats', const Color(0xFF2563EB))),
                        const Gap(12),
                        Expanded(child: _statCard('Vacant', '${_totalSeats - _bookedSeats}', AppColors.gray400)),
                        const Gap(12),
                        Expanded(child: _statCard('Total', '$_totalSeats', AppColors.primary)),
                      ]),
                      const Gap(24),
                      ...
                      // Seat Grid
                      if (_seats.isEmpty)
                        const Center(child: Padding(padding: EdgeInsets.all(40), child: Text('No seat data', style: TextStyle(color: AppColors.gray400))))
                      else
                        _buildSeatGrid(),
                    ]),
                  ),
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child: const Center(child: Text('🚌 DRIVER', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.primary))),
        ),
        const Gap(12),
        // Seat rows
        ...
        Container(
          ...
          child: const Center(child: Text('REAR', style: TextStyle(fontSize: 9, color: AppColors.gray500))),
        ),
      ]),
    );
  }
  ...
  Widget _seatCell(Map<String, dynamic> seat) {
    ...
    return Expanded(
      child: Container(
        ...
        child: Center(child: Text(num, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: color))),
      ),
    );
  }
